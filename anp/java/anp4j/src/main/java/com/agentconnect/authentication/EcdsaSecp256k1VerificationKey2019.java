...
public class EcdsaSecp256k1VerificationKey2019 extends VerificationMethod {
    ...
    public boolean verifySignature(byte[] content, String signature) {
        try {
            // Decode base64url signature (R|S format)
            byte[] signatureBytes = decodeBase64Url(signature);

            // Convert R|S format to r, s BigIntegers
            ...
            // The content passed here is already the first SHA256 hash (from DIDWBA.getSignatureResult)
            // The signing process uses SHA256withECDSA which does SHA256(content) internally
            // So we need to hash once more to match: SHA256(first_hash) = double_hash
            MessageDigest digest = MessageDigest.getInstance("SHA-256");
            byte[] hashToVerify = digest.digest(content);

            // ECDSASigner operates on raw hash - no additional hashing
            ECDSASigner signer = new ECDSASigner();
            signer.init(false, publicKey);
            return signer.verifySignature(hashToVerify, r, s);
